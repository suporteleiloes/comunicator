...
const Cronometro = {
  ...
  computed: {
    timerPregao () {
      ...
      if (Number(this.lote.status) < 5 || Number(this.lote.status) > 10000) {
        // Ativa cronômetro
        // const pregao = this.lote.historicoPregao && Array.isArray(this.lote.historicoPregao) ? this.lote.historicoPregao.find(h => !h.dataEncerramento) : null
        if (!this.timeUltimaAtividade || !this.timeLimite) {
          return timer
        }
